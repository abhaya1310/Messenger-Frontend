...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ orgId: string; outletId: string }> }) {
    const authHeaders = await getAdminAuthHeaders(request);
    if (!authHeaders) {
        return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const { orgId, outletId } = await ctx.params;
    ...
    const res = await fetch(
        `${getBackendBaseUrl()}/api/admin/org/${encodeURIComponent(orgId)}/outlets/${encodeURIComponent(outletId)}/pos`,
        {
            method: 'PATCH',
            headers: {
